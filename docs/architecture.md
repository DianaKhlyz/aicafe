...
| Фронтенд | **Vite SPA + пререндер публичных страниц** | Интерактив (корзина, чекаут, карта зала) — SPA; SEO-страницы (~10–20 штук) пререндерятся в статический HTML при билде и по вебхуку | 
| Типизация | Pydantic на бэке → OpenAPI → **сгенерированные TS-типы** на фронте (`openapi-typescript`) | Общий контракт типов без монолита: поменял модель — фронт не соберётся, пока не поправишь. *(TS на фронте — предварительно, см. открытые вопросы)* |
